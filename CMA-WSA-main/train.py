...
def train(_config, train_dataset, val_dataset, test_dataset, device, question_vocab):
    ...
    for epoch in range(num_epochs):
        ...
        if oneStep and epoch >= _config["thread_epoch"] and not called:
            ...
            for param in model.maskNet.parameters():
                param.requires_grad = False
        # After reaching thread_epoch, freeze WaveletIIA parameters and switch to eval()
        if epoch >= _config["thread_epoch"]:
            if hasattr(model, "wavelet_iia_module"):
                model.wavelet_iia_module.eval()
                for p in model.wavelet_iia_module.parameters():
        ...
        for i, data in tqdm(
            ...
            # Compute losses based on whether MoE is used
            # In the loss computation section
            use_moe = _config.get('fusion_mode', 'moe').lower() == 'moe'
            
            if use_moe and prob is not None:
                step_moe_loss = moe_loss.item()
                moeLoss += step_moe_loss
                # Expert statistics
                expert_count += torch.sum(prob, dim=0).detach().cpu().numpy()
            else:
                step_moe_loss = 0.0
            
            # Loss computation
            if use_moe:
                loss = _config["alpha"] * rmse + (1 - _config["alpha"]) * acc_loss + moe_loss
            else:
                loss = _config["alpha"] * rmse + (1 - _config["alpha"]) * acc_loss
            ...
            step_moe_loss = 0  # default value
            if use_moe:
                step_moe_loss = moe_loss.cpu().item()
            # Adjust expert statistics handling 
            if prob is not None and use_moe:
